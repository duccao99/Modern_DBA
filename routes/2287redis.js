...
const getBook = async (req, res) => {
    ...
    try{
        ...
        client.setex(isbn, 360, JSON.stringify(result));
...
router.get("/product/:productId",getCache);
router.get("/product2/:productId",(req, res) => {
    const isbn = req.params.productId;
    try {
        client.get(isbn, async (err, result) => {
            if (err) throw err;
            ...
            if (result) {
                ...
                const result = await Dbquery.getDetail(isbn);
                client.setex(isbn, 600, JSON.stringify(result));
                res.status(200).send({
                    jobs: result,
                    message: "cache miss"
                });
...
router.get("/product3/:categoryId",(req, res) => {
    const isbn = req.params.categoryId;
    const skip = req.query.skip || 0;
    const limit = req.query.limit || 0;
    try {
        client.get(`${isbn}:${skip}:${limit}`, async (err, result) => {
            if (err) throw err;
            ...
            if (result) {
                ...
                const result = await Dbquery.getByCategory(isbn);
                client.setex(`${isbn}:${skip}:${limit}`, 600, JSON.stringify(result));
                res.status(200).send({
                    jobs: result,
                    message: "cache miss"
                });
